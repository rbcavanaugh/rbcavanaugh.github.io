...
t = CV$entries_data %>%
  ...
  fill(start) %>%
  # separate(title, into = c("authors", "title"), sep = "\\([^)]*\\).") %>%
  # mutate(title = str_trim(title)) %>%
  # separate(title, into = "title", sep = "[.|?]") %>%
  rowwise() %>%
  ...
  #mutate(out = glue::glue('{title}<br><small>{authors}</small>'))%>%
  select(
   # Year = start,
    Article = title)
    #Article = title,
...
htmltools::div(style="min-height:50vh;", 
DT::datatable(t,
              escape = FALSE,
              rownames = FALSE,
              class="compact hover",
